...
impl UserSystem {
    pub async fn new(core: SharedCore, s3: S3Ref) -> Self {
        ...
        let require_admin = !bbs_utils::exists_table(tx.as_mut(), "users").await;
        sqlx::query(
        ...
        let mut this = UserSystem {
            s3,
            core: core.clone(),
            cached_users: bbs_utils::get_cache_instance(config.clone()).await,
            cached_users_array: bbs_utils::get_cache_instance(config.clone()).await,
            create_task: None,
            check_task: None,
        };
    ...
    pub async fn revert(&self, auth: &str) -> Result<Option<UserInfo>> {
        ...
        match auth_row {
            Ok(r) => {
                ...
                if created_at <= bbs_utils::get_last_active_time(auth_active_days).await {
