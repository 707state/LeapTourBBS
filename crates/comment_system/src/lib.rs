...
impl CommentSystem {
    ...
    pub async fn new(core: SharedCore) -> Self {
        ...
        let mut this = CommentSystem {
            core,
            cached_comments: bbs_utils::get_cache_instance(config.clone()).await,
            cached_comments_array: bbs_utils::get_cache_instance(config.clone()).await,
            create_task: None,
        };
        let cs = this.clone();
    ...
    pub async fn create_comments(
        ...
        if r.rows_affected() == (len as u64) {
            ...
            for (post_id, (i, total_comment, total_comment_post)) in reply_posts {
                bbs_utils::increment_post_total_comment(
                    tx,
                    post_id,
            ...
            for (comment_id, (i, total_comment)) in reply_comments {
                bbs_utils::increment_comment_total_sub_comments(
                    tx,
                    comment_id,
                    comments[i].created_at,
                    comments[i].created_by_id,
            ...
            for (user_id, total_comment) in user_id_map {
                bbs_utils::increment_user_total_comment(&mut *tx, user_id, total_comment).await?;
            }
            Ok(comments)
        } else {
            bail!("Insert failed.")
