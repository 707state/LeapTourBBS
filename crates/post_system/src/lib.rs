...
impl PostSystem {
    pub async fn new(core: SharedCore, s3: S3Ref) -> Self {
        ...
        let mut this = PostSystem {
            core,
            s3,
            cached_posts: bbs_utils::get_cache_instance(config.clone()).await,
            cached_posts_array: bbs_utils::get_cache_instance(config.clone()).await,
            cached_posts_count: bbs_utils::get_cache_instance(config.clone()).await,
            create_task: None,
            views_task: None,
        };
        let ps = this.clone();
    ...
    pub async fn create_posts(
        ...
        query_builder.push_values(&posts, |mut b, (user_id, post)| {
            b.push_bind(user_id)
                ...
                .push_bind(bbs_utils::array_to_string(&post.tags))
                .push_bind(now)
                .push_bind(now)
                .push_bind(user_id)
        ...
        if r.rows_affected() == (len as u64) {
            ...
            for (category_id, total_post) in category_id_map {
                bbs_utils::increment_category_total_post(&mut *tx, category_id, total_post).await?;
            }
            for (user_id, total_post) in user_id_map {
                bbs_utils::increment_user_total_post(&mut *tx, user_id, total_post).await?;
            }
            Ok(p)
        } else {
            bail!("Insert failed.")
    ...
    pub async fn update_post(
        ...
        .bind(bbs_utils::array_to_string(&post.tags))
        .bind(&now)
        .bind(user_id)
        .bind(&post.cover_url)
        .bind(post.top_index)
    ...
    async fn from_row(&self, row: SqliteRow, fetch_content: bool) -> PostInfo {
        ...
        PostInfo {
            ...
            tags: bbs_utils::string_to_array(row.try_get("tags").unwrap()).unwrap(),
