...
impl LikeSystem {
    ...
    pub async fn get_like_status(
        ...
        Ok(
            sqlx::query_as::<_,LikeStatus>("SELECT * FROM all_like_status WHERE flag_ref_id = ? AND flag = ? AND created_by_id = ? LIMIT 1",
).bind(flag_ref_id)
            .bind(flag)
            .bind(user_id)
        ...
        for ((flag, flag_ref_id), bundles) in bundles_by_map {
            ...
            match flag {
                ...
                LikeStatusFlag::TargetComment => {
                    bbs_utils::increment_comment_like(
                        tx,
                        flag_ref_id,
                        total_like_insert,
                        total_dislike_insert,
