...
impl CategorySystem {
    pub async fn new(core: SharedCore, s3: S3Ref) -> Self {
        ...
        let this = CategorySystem {
            core,
            s3,
            cached_categories: bbs_utils::get_cache_instance(config.clone()).await,
            cached_categories_array: bbs_utils::get_cache_instance(config).await,
        };
        this
    }
    ...
    pub async fn is_user_inside_group(
        ...
        if sqlx::query("SELECT 1 FROM category_groups WHERE category_id=?")
            .bind(category_id)
            .fetch_optional(&mut *tx)
            .await?
            .is_none()
