...
impl VerificationSystem {
    pub async fn new(core: SharedCore, s3: S3Ref) -> Self {
        ...
        let mut this = VerificationSystem {
            s3,
            image_format: ImageFormat::from_extension(core.get_config().image_format.as_ref())
    ...
    pub async fn get_verifications(
        ...
        if r.rows_affected() == length as u64 {
            ...
            for (i, captcha) in cs.into_iter().enumerate() {
                ...
                captcha
                    .image
                    .write_to(&mut Cursor::new(&mut content_bytes), self.image_format)?;
                let key = {
                    let hash = bbs_utils::calc_hash(&content_bytes);
    ...
    pub async fn set_verification_passed(
        ...
        Ok(match r {
            Some(v) => {
                ...
                let created_at = chrono::Utc.timestamp_opt(v.create_at as _, 0).unwrap();
